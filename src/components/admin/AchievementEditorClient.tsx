...
export default function AchievementEditorClient({ id }: { id?: string }) {
      ...
      const fetchAchievement = async () => {
        try {
          const response = await axios.get(
            `${INTERNAL_API_BASE_URL}/achievements/${id}`
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id && id !== "new") {
        ...
        await axios.post(`${INTERNAL_API_BASE_URL}/achievements`, formData, {
          headers,
        });
    ...
    <div className="text-neutral-900 dark:text-neutral-100 max-w-4xl mx-auto p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold">
          {id && id !== "new" ? "Edit Achievement" : "Add Achievement"}
        </h1>
      </div>
